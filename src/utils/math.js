...
const calculateDistance = (a, b) => Math.sqrt((Math.pow(b.x, 2) - Math.pow(a.x, 2)) + (Math.pow(b.y, 2) - Math.pow(a.y,2)));
const vectorMagnitude = (x, y) => Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2));
const angleToPoint = (x1, y1, x2, y2) => {
    const d = calculateDistance({x: x1, y: y1}, {x: x2, y: y2});
...
const rotateVectorByAngle = ({x, y}, angle) => ({
